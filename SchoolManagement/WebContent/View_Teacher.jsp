...
int j=0;
try{
	j=(Integer)session.getAttribute("c_id");
}
catch(Exception e)
{
...
SELECT  a_id ,a_name
   FROM student_info s, teacher_info a
   WHERE a.a_id=<%=j %> LIMIT 1;
...
{  
    ...
    Connection con=DriverManager.getConnection("jdbc:mysql://localhost/samyak_sms?verifyServerCertificate=false&useSSL=true","samyak_samyak","9210488110@acp");
 	PreparedStatement ps=con.prepareStatement("select t_id,t_name FROM teacher_info  ");
...
<td><c:out value="${table.a_id}"/></td>  
<td><c:out value="${table.a_name}"/></td>  
